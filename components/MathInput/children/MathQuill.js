...
export default function MathQuill({
  ...
  if (!memKey) {
    throw new Error(
      "Each MathInput component needs a unique memKey for storing quetsion data in memory"
    );
  }

  const [firstRender, setFirstRender] = useState(true);

  const marksEntry = useTransition(
    memory.feedbackShown,
    firstRender
      ? { enter: { x: "0px" }, leave: { x: `57px` } }
      : {
          from: { x: `57px` },
          enter: { x: "0px" },
          leave: { x: `57px` },
  ...
  return (
    <div className="mathInput">
      <div
        ...
        className={styles.mathFieldWrapper}
        onFocus={() => {
          setButtonsShown(true);
          if (memory.feedbackShown) {
            setMemory({ feedbackShown: false });
          }
        }}
        onBlur={() => {
      ...
      >
        <div className={styles.wrapper}>
          {memory[memKey] && memory[memKey].score === 1
            ? marksEntry((style, item) => {
                return item ? (
                  <animated.div style={style} className={styles.correctIcon}>
                    <Image
                      alt="correct"
                      src={thickWhiteTick}
                      layout="responsive"
            ...
            : marksEntry((style, item) => {
                return item ? (
                  <animated.div style={style} className={styles.incorrectIcon}>
                    <Image
                      alt="incorrect"
                      src={thickWhiteCross}
                      layout="responsive"
          ...
          <EditableMathField
            ...
            mathquillDidMount={mountingHandler}
          />
        </div>
        {buttonsShown || latex !== "" ? (
          ""
      ...
      {buttonsShown && buttonArray && buttonArray.length > 0 ? (
        ...
        >
          ...
          {buttonArray.includes("times") ? (
            ...
            >
              <div className={styles.clickBlocker}>
                <StaticMathField>{"\\times"}</StaticMathField>
              </div>
            </button>
          ) : (
            ""
          ...
          {buttonArray.includes("coordinates") ? (
            <button
              onMouseDown={(e) => {
                e.preventDefault();
              }}
            ...
            >
              <div className={styles.clickBlocker}>
                <StaticMathField>{"\\left(x, \\ y\\right)"}</StaticMathField>
              </div>
            </button>
          ) : (
            ""
          )}
          {buttonArray.includes("pound") ? (
            <button
              onMouseDown={(e) => {
                e.preventDefault();
              }}
          ...
          {buttonArray.includes("vector_a") ? (
            ...
            >
              <div className={styles.clickBlocker}>
                <StaticMathField>{"\\mathrm{a}"}</StaticMathField>
              </div>
            </button>
          ) : (
            ""
          )}
          {buttonArray.includes("vector_b") ? (
            <button
              onMouseDown={(e) => {
                e.preventDefault();
              }}
            ...
            >
              <div className={styles.clickBlocker}>
                <StaticMathField>{"\\mathrm{b}"}</StaticMathField>
              </div>
            </button>
          ) : (
            ""
